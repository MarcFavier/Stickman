...
def training_loop(DNN, head, epochs, optimizer, learning_rate, loss_fn, train_set, val_set, transform_labels, metric):
	...
	for epoch in range(epochs):
		...
		logits = DNN(images, training=False)
		print("\n")
		print("\rKP",KP[1])
		print("\rlogits",logits[1])
		loss_value_val = loss_fn(transform_labels(KP, logits.shape[-2]), logits)
		loss_value_val += sum(DNN.losses)
		loss_value_val *= learning_multiplier
		
		#modifier pour faire un truc plus joli
		print("\rvalidation is ", loss_value_val)
		print("\n")
	print()
	return DNN
