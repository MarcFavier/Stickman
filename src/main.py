...
from save.saver import save_prediction_test
from save.collect import get_trained_DNN
from visu.show import visu_eval_DNN
from evaluation_test.test import compute_test_pred
from evaluation_test.test import compute_test_pred, eval_DNN
...
parser.add_argument('-head', choices=possible_heads, nargs='?', type=str, default='mob_heads')
parser.add_argument('-backbone', choices=possible_backbones, nargs='?', type=str, default='MOBILE')
...
parser.add_argument('-epochs', nargs='?', type=int, default=35)
parser.add_argument('-batch_size', nargs='?', type=int, default=64)
parser.add_argument('-lr', nargs='?', type=float, default=0.001)#0.001avant
...
parser.add_argument('-load', nargs='?', type=bool, default=True)#True pour charger
parser.add_argument('-continue', nargs='?', type=bool, default=False)#True pour continuer l'entrainement d'un model
...
if not args.load:
	...
	transfo = {
		...
		'mob_heads' : transform_labels_scalars
	}
...
pred_mat = compute_test_pred(DNN=DNN, image_shape=image_shape)
save_prediction_test(pred_mat)
print(len(pred_mat))
print(len(pred_mat[1]))
